...
var list1 = debtors.Where(dt => dt.Email.EndsWith("rhyta.com") || dt.Email.EndsWith("dayrep.com")).ToList();



var list2 = debtors.Where(dt => DateTime.Now.Year - dt.BirthDay.Year >= 26 && DateTime.Now.Year - dt.BirthDay.Year <= 36).ToList();


var list3 = debtors.Where(dt => dt.Debt <= 5000).ToList();


var list4 = debtors.Where(dt => dt.FullName.Length >= 18 && dt.Phone.Count(ch => ch == '7') >= 2).ToList();



var list5 = debtors.Where(dt => dt.BirthDay.Month == 12 || dt.BirthDay.Month == 1 || dt.BirthDay.Month == 2).ToList();


var list6 = debtors.Where(dt => dt.Debt > debtors.Average(dt => dt.Debt)).OrderByDescending(dt => dt.FullName.Split()[2]).OrderByDescending(dt => dt.Debt).ToList();


var list7 = debtors.Where(dt => !dt.Phone.Contains('8')).ToList();


var list8 = debtors.Where(dt =>
{
    for (int i = 0; i < dt.FullName.Length; i++)
    {
        int sum = 0;
        for (int j = 0; j < dt.FullName.Length; j++)
        {
            if (dt.FullName[i] == dt.FullName[j])
            {
                sum++;
            }
        }
    ...
    return false;
}).OrderByDescending(dt => dt.FullName).ToList();

list8.ForEach(dt => Console.WriteLine(dt));


var list9 = debtors.Where(dt => dt.Debt == debtors.Max(dt => dt.Debt)).ToList();


var list10 = debtors.OrderByDescending(dt => dt.Debt).ToList();


var list12 = debtors.Where(dt => dt.BirthDay.Year <= 1945).ToList();

var list13 = debtors.Where(dt =>
{
    bool cond = true;
    for (int i = 0; i < dt.Phone.Length; i++)
    {
        int sum = 0;
        for (int j = 0; j < dt.Phone.Length; j++)
        {
            if (dt.Phone[i] == dt.Phone[j])
            {
                sum++;
            }
        }
        cond = sum == 1 ? true : false;
        if (cond == false)
            return false;
    }
...
var list14 = debtors.Where(dt =>
{
    DateTime date = DateTime.Now;
    while (date.Month != dt.BirthDay.Month)
    {
        dt.Debt -= 500;
        date = date.AddMonths(1);
    }
    dt.Debt -= 500;
    return dt.Debt <= 0 ? true : false;
}).ToList();

var list15 = debtors.Where(dt => dt.FullName.ToLower().Contains("smile")).ToList();